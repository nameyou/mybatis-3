...
public class DynamicSqlSource implements SqlSource {
  ...
  public BoundSql getBoundSql(Object parameterObject) {
    ...
    //通过调用 rootSq lNode.apply （）方法调用整个树形结构中全部 SqlNode. apply （）方法，读者可以
    // 体会一下组合设计模式的好处。 每个SqlNode 的 apply（）方法都将解析得到的 SQL 语句片段追加到
    //context中， 最终通过 context .getSql（）得到完整的 SQL 语句
